...
function numbersplit(number){
	...
	if(getnumbersym>2 && number.length<5){
		return 3;
...
function CheckBet(send_data){
	send_data_final = {};
	var idx = 0;
	for(var i in send_data){
		var classid = GetClassid(send_data[i].number,send_data[i].statsizi);
		var max_per_bet=_cacheuser[classid]['o'];
		rest_money = send_data[i].money;
		while(Number(rest_money) > Number(max_per_bet)){
			var tmp_obj = {};
			tmp_obj.id = idx;
			tmp_obj.number = send_data[i].number;
			tmp_obj.money = max_per_bet;
			tmp_obj.statsizi = send_data[i].statsizi;
			send_data_final[idx] = tmp_obj;
			idx++;
			rest_money = rest_money - max_per_bet;
		}
		var tmp_obj = {};
		tmp_obj.id = idx;
		tmp_obj.number = send_data[i].number;
		tmp_obj.money = rest_money;
		tmp_obj.statsizi = send_data[i].statsizi;
		send_data_final[idx] = tmp_obj;
		idx++;
	}
...
function GetClassid(number,statsizi){
	var return_classid = 0;
	if(number == '') return return_classid;
	number = number.replace(/[\d]/g,"口");
	number = number.toUpperCase();
	for(var i in _cacheclass){
		var classname = _cacheclass[i]['classname'];
		var parentid = _cacheclass[i]['parentid'];
		
		if(parentid > 0 && classname == number){ 
			return_classid = _cacheclass[i]['id'];
			break;
	...
	if(return_classid == 0){
		if(number.length == 2){
			...
			return_classid = 7;
		}else if(number.length == 4 && statsizi == 1){
			return_classid = 107;
		}else if(number.length == 4 && statsizi == 0){
			return_classid = 5;
	...
	return return_classid;
}

function sortnumber(number,sizixian){
	if(number.length < 4 || (number.length==4 && sizixian==1)){
		var temp_array = [];
		for(var i=0;i<number.length;i++){
			temp_array[i] = number.substr(i,1);
		}
		var sort_number = '';
		temp_sort = temp_array.sort();
		for(var i=0;i<temp_sort.length;i++){
			sort_number += temp_sort[i];
		}
		return sort_number;
	}else{
		return number;
