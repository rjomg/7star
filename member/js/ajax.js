...
function post_soonMsg(str,id){
	...
	if(htmlstr==undefined){
		if(htmlstrnum==3){
			...
			*/
			...
			//alert(htmlstr)
			XMLHttp.sendReq("POST","./ajax/ajax.php?action=resetOrder","inajax=1",post_soonMsg,"showorderhtml");
		}
		htmlstrnum++;
		return;
	}else if(erroe==4){ 
	...
	if(erroe==1){
		...
		// alert("下注成功筆數： " + s_arr[12] + "筆， 失敗筆數：" + s_arr[13] + "筆。");
		showorderhtml(htmlstr,s_arr[8],s_arr[9]);
...
function showorderhtml(arr,s_arr8,s_arr9){
	...
	if(in_wapcg!=1&&_sendmode==1&&(_styles=='trad'||_styles=='v2_trad'||_styles=='v2_trad_1')){//2组风格左边打印部分
		...
		if(in_wapcg!=1){//1组风格左边打印部分
			var objmenu_left = window.parent.parent.frames["menu"].document.getElementById("showorderhtml");
			if(objmenu_left){	
...
function rcedits(leavings,use,now){
	if(in_wapcg==1){
		...
		var objmenu = window.parent.parent.frames["menu"];
		if(objmenu){
			objmenu.document.getElementById("my_rcedits_leavings").innerHTML=leavings;
			objmenu.document.getElementById("my_rcedits_use").innerHTML=use;
			objmenu.document.getElementById("my_rcedits").innerHTML=now;
...
//for ajax
function ajax(method, url, sync, data, callback){
	var request = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("MSXML2.XMLHTTP.3.0");
	request.onreadystatechange = function(){
		if (request.readyState == 4 && request.status == 200){
			response_data = request.responseText;
			(callback && typeof(callback) === "function") && callback(response_data);
		}
	};
