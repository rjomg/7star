...
function sendAjax(url, types, param, datat, callback, beforeSend, complete) {
	return ajaxSendPost = $.ajax({
		timeout: 3 == getIsPC ? 5e3: 0,
		type: types,
		url: url,
		data: param,
		dataType: datat,
...
function IsPC() {
	...
	return /iphone|ipad|ipod/.test(ua) ? flag = 1 : /baidu/.test(ua) || /vivobrowser/.test(ua) ? flag = 3 : /android/.test(ua) && (flag = 2),
	flag
}
...
cgCheckSend = {
	...
	isDianNum: function(n) {
		var money = n + "",
		s = 0;
		if (money.indexOf(".") != -1) {
			var arr = money.split(".");
			if (arr.length > 1) {
				var zuihou = parseInt(arr[1]);
				if (zuihou > 0) {
					s = (zuihou + "").length
				} else s = 0
			}
	...
	CacheMoney: function() {
		...
		var zuixiao = l,
		zuixiao_num = this.isDianNum(zuixiao),
		money_num = this.isDianNum(money);
		return getmoney > o ? (s = 1, cgCheckSend.moneymsg = o) : getmoney < l ? (s = 2, cgCheckSend.moneymsg = l) : !this.isDian(zuixiao) && this.isDian(money) ? s = 3 : zuixiao_num > 0 && money_num > 0 && zuixiao_num != money_num && (s = 7),
		s
	},
	__moneyall: 0,
...
var cgSelect = {
	...
	GetSoonSelect: function(data) {
		...
		function() {});
			...
			cgSelect.logid > 0 || json.logsszd <= 0 && (setTimeout("$('#selectlogsclassid').val(1);", 100), setTimeout("cgSelect.__showmeun(1);", 100))
		} else jAlert("未知错误", "提示框",
		function() {})
	},
	but_start: function() {
	...
	but_stop: function() {
		$("#sendsoonselect_but").attr("disabled", !0)
	},
	isChack: function() {
		...
		if (200 == stats) {
			if (stats = cgCheckSend.CacheMoney(), 200 == stats) return cgSelect.lujingstat = 3,
			$(this).attr("disabled", !0),
			this.SendSoonSelectInit(),
			!1;
			cgCheckSend.showMoney(stats,
			function() {
				return $("#" + cgSelect.__mID).select(),
				!1
			})
		} else cgCheckSend.showNumber(stats,
		function() {
			return ! 1
		});
		return s = "",
	...
	SendSoonSelectInit: function() {
		...
		return this.pages = this.getNumberCount / this.pagenum + b,
	...
	SendComplete: function(XMLHttpRequest, status) {
		"timeout" == status && (ajaxSendPost.abort(), jAlert("已下注成功，请返回。", "提示框",
		function() {
			setTimeout("window.location.href='/?action=logout';", 200)
		}))
	},
	SendSoonSelect: function() {
		return 1 == cgSelect.sendstat ? (jAlert("数据正在处理,请稍等", "提示框",
		function() {
			$("#windowClose").hide()
		}), !1) : (sendAjax("appindexajax.php", "post", "action=soonselectnumber&post_number_money=" + cgSelect.mNumber + "&post_money=" + cgSelect.mMoney + "&sizixian=" + cgSelect.xian + "&selectlogsclassid=" + cgSelect.selectlogsclassid + "&zjzc=" + cgSelect.zjzc + "&selectnumbertotal_hidden=" + cgSelect.selectnumbertotal_hidden + "&selectlogs=" + cgSelect.selectlogs + "&strarray=" + cgSelect.strarray + "&lujingstat=" + cgSelect.lujingstat + "&versionName=" + cgSelect.versionName, "json", cgSelect.SendSoonSelectFunc, cgSelect.setEmpty, cgSelect.SendComplete), !1)
	},
	setEmpty: function() {
		cgSelect.sendstat = !0,
		cgSelect.but_stop()
	...
	SendSoonSelectFunc: function(data) {
		...
		function() {
			cgSelect.sendstat = !1,
			cgSelect.but_start()
		}) : (cgSelect.pages = cgSelect.pages - 1, 200 == s && (cgSelect.credits_use = json.credits_use, cgSelect.updateProgressBar(100, "发送:" + cgSelect.getNumberCount + "个号码完成！"), LeftPintIframe.refresh(), jsonAjax("appindexajax.php", "get", "action=soonorder&diyici=1", "json", cgOrber.GetSoonOrder), $(".top_kaida li [rel=tab_kuaida]").click(), cgSelect.setVal(), setTimeout("$.alerts._hide();", 1e3)))
	},
	showProgressBar: function() {
		return jProgressBar("", "快选进度条",
		function() {
			var getSpNum = parseInt(cgSelect.pagesOercentage * cgSelect.pagesAdd);
			cgSelect.updateProgressBar(getSpNum, "正在处理<span>请稍等</span>")
		}),
		!1
	},
	updateProgressBar: function(num, title) {
		$(".ProgressBar_t").html(title),
	...
	init: function() {
		...
		$("#sendsoonselect_but").unbind("click").click(function() {
			return 0 == cgSelect.sendstat && cgSelect.isChack(),
			!1
		}),
...
var __ss = {
	...
	__int_dingwei_num: function() {
		...
		var sjstat = !1;
		for ("" != this.__shangjiang && this.__dingwei_chu && (sjstat = !0), i = 0; i < this.__dingweiarray.length; i++) if (this.__dingwei_shangjiang_arr[i] = "", "" != this.__dingweiarray[i]) {
			...
			chustr = data;
			if (num.length > 0) {
				for (sjstat && (getdata = data.split("")), j = 0; j < num.length; j++) gnum = num.slice(j, j + 1),
				arrnum += comm + gnum,
				arrfuhao = "0-9",
				comm = "|",
				chustr = chustr.replace(gnum, ""),
				sjstat && getdata.splice(gnum, 1, "");
				if (sjstat) {
					var arrfuhao = "";
					for (h = 0; h < getdata.length; h++) arrfuhao += getdata[h];
		...
		1 == this.__classid && 1 == data.length || 2 == this.__classid && data.length <= 2 || 3 == this.__classid && data.length <= 3 ? (this.__dingwei_exits = !0, this.__dingwei_reg += "[0-9" + this.__fuhao + "]", this.__dingwei_shangjiang_reg += "[0-9" + this.__fuhao + "]") : (this.__dingwei_reg += "[" + data + this.__fuhao + "]", this.__dingwei_shangjiang_reg += "[" + data + this.__fuhao + "]"),
	...
	__check_dingwei_reg: function(n) {
		...
		if ("" != this.__shangjiang) {
			this.__dingwei_chu && (this.__dingweiarray = this.__dingwei_shangjiang_arr),
			...
			if (1 == this.__classid) {
				...
				shangjiangreturn = !(!(dwnum > 0 && dwnum <= datanum || dwnum == datanum || data.length <= datanum) || !setData2true)
...
cgDetail = {
	...
	detailList: function(json) {
		...
		function(ii, item) {
			...
			showtuimadate = "",
			yansehtml = "";
			yanse > 0 && (yansecss = 1 == yanse ? "#339900": "#ff0000", yansehtml = (printnumber > 0 ? printnumber + "次": "") + "<font color='" + yansecss + "' size='4'>●</font>"),
			1 != statsizi && 6 != classid && 7 != classid || (sizitext = "现"),
			1 == stattuima ? (tuimatext = "--", hotcss = 'class="tuima"', stattext = "退码", "detailprint" != json.doaction && (showtuimadate = "<br>退" + tuimatime)) : (1 == hotstat && (hotcss = 'class="hotcss"'), tuimatext = 2 == tuima || 3 == tuima ? '<input id="checkboxID' + id + '" type="checkbox" value="' + id + '"/>': "--", getnum++),
			"detailprint" == json.doaction ? html += "<tr " + hotcss + "><td>" + yansehtml + cgOrber.caizhongarr[cgOrber.caizhongselect] + "</td><td>" + orderid + "</td><td>" + datetime + showtuimadate + '</td><td class="number">' + number + "<span>" + sizitext + '</span></td><td class="money">' + money + '</td><td class="frank">' + frank + "</td><td >" + hitaward + "</td><td>" + stattext + "</td></tr>": (html = "<tr " + hotcss + "><td>" + yansehtml + cgOrber.caizhongarr[cgOrber.caizhongselect] + "</td><td>" + orderid + "</td><td>" + datetime + showtuimadate + '</td><td class="number">' + number + "<span>" + sizitext + '</span></td><td class="money">' + money + '</td><td class="frank">' + frank + "</td><td >" + hitaward + "</td><td >" + huishui + "</td><td >" + yingkui + "</td><td>" + stattext + "</td><td>" + tuimatext + "</td></tr>", $("#" + cgDetail.divname).append(html))
		}), allmoney = Number(json.allmoney), allhitaward = Number(json.allawrdmoney), allhuishui = Number(json.allhuishui), allyingkui = Number(json.allyingkui), "detailprint" == json.doaction) {
			var showtitle = "";
			"award" == json.joaction ? (showtitle = "中奖明细", cgOrber.showtit1 = "小票编号", cgOrber.showtit2 = "小票时间") : (showtitle = "下注明细", cgOrber.showtit1 = "注单编号", cgOrber.showtit2 = "下单时间"),
			html += '<tr id="detailheji"><td colspan=2>合计</td><td>' + getnum + "</td><td > </td><td>" + allmoney + "</td><td> </td><td> </td></tr>",
			cgDetail.detailPrint(html, "第" + (json.issueno > 0 ? json.issueno: json.issueno_now) + "期 " + json.printdate + "   " + showtitle)
		} else html = '<tr id="detailheji"><td colspan=2>合计</td><td>' + getnum + "</td><td > </td><td>" + allmoney + "</td><td> </td><td >" + allhitaward + "</td><td >" + allhuishui + "</td><td >" + allyingkui + "</td><td> </td><td> </td></tr>",
		$("#" + cgDetail.divname).append(html);
		dlist = "",
		html = ""
	},
	showtit1: "",
	...
	detailPrint: function(html, ttt) {
		var html = '<table class="alertsPrint1" width="100%" align="center">  <tbody><tr><td width="10%">彩种</td><td width="20%">' + cgOrber.showtit1 + '</td><td width="*">' + cgOrber.showtit2 + '</td><td width="10%">号码</td><td width="10%">金额</td><td width="10%">赔率</td><td width="10%">中奖</td><td width="10%">状态</td></tr>' + html + "</tbody></table>";
	...
	setValInit: function() {
		...
		$("#ds_s_money").val(""),
		$("#ds_s_money_end").val(""),
		cgDetail.setValInitSelct("#soclass"),
		cgDetail.setValInitSelct("#so_s_classid")
	},
	setValInitSelct: function(n) {
		$(n).each(function(i, j) {
			$(j).find("option:selected").attr("selected", !1),
			$(j).find("option").first().attr("selected", !0)
		})
	},
	setSearch: function() {
	...
	init: function() {
		...
		$("#detailprint").unbind("click").click(function() {
			var setsan = cgDetail.setSearch();
			jsonAjax("appindexajax.php", "get", "action=detail&doaction=detailprint&joaction=" + cgDetail.joaction + "&page=" + cgDetail.pages + setsan, "json", cgDetail.GetDetail)
		}),
		$("#detailsearch").unbind("click").click(function() {
			cgDetail.doaction = "detailprint" == cgDetail.doaction ? "": cgDetail.doaction;
			var setsan = cgDetail.setSearch();
			jsonAjax("appindexajax.php", "get", "doaction=" + cgDetail.doaction + "&action=detail" + setsan, "json", cgDetail.GetDetail)
		}),
...
cgBill = {
	...
	GetBill: function(data) {
		...
		function() {});
			...
			cgBill.billList(json.billitem);
			var s_issueno_start = json.s_issueno_start,
			s_issueno_end = json.s_issueno_end,
			arrIssueno = json.getIssueNoSelect,
			s_i = cgBill.issuenoHtml(arrIssueno, "s_issueno_start", s_issueno_start),
			s_e = cgBill.issuenoHtml(arrIssueno, "s_issueno_end", s_issueno_end);
			$("#bill_issueno_start").html(s_i),
			$("#bill_issueno_end").html(s_e),
			1 == json.pages && cgBill.setPage(json.zallnum - 0, json.allpage - 0, json.numpage - 0)
		} else jAlert("未知错误", "提示框",
		function() {})
	},
	setPage: function(allnum, allpage, numpage) {
	...
	setVal: function() {
		_self = this,
		_self.s_issueno_start = $("#s_issueno_start").val(),
		_self.s_issueno_end = $("#s_issueno_end").val()
	},
	setSearch: function() {
		return cgBill.setVal(),
		"&s_issueno_start=" + cgBill.s_issueno_start + "&s_issueno_end=" + cgBill.s_issueno_end
	},
	Search: function() {
		jsonAjax("appindexajax.php", "get", "action=bill" + cgBill.setSearch(), "json", cgBill.GetBill)
	},
	issuenoHtml: function(i, name, s_issueno) {
		var dlist = i.i,
		ddate = i.d,
		html = '<select name="' + name + '" id="' + name + '" onchange="cgBill.Search();" >',
		arrnum = dlist.length - 1;
		return 0 != isNull(dlist) && $.each(dlist,
		function(ii, item) {
			var s = "",
			v = dlist[ii];
			"s_issueno_start" == name ? s = s_issueno > 0 ? s_issueno == v ? 1 : 0 : arrnum == ii ? 1 : 0 : (s = s_issueno == v ? 1 : 0, s_issueno <= 0 && dlist[0] == v && (s = 1));
			var d = ddate[v];
			showselect = 1 == s ? "selected": "",
			d = null == d ? "": d,
			html += '<option value="' + v + '" ' + showselect + ">" + d + "(" + v + ")</option>"
		}),
		html += "</select>"
	},
