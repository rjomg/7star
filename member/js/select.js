var __ss={
	...
	__shows:function (){
		...
		if( parent.window.frames[0].document.getElementById("sizixian")){		
			parent.window.frames[0].document.getElementById("sizixian").checked=false;
		}
		...
		if(this.__classid==1){
			...
			if(this.__classid==6){
				...
				$("ss7").style.display="";	
				if(parent.window.frames[0].document.getElementById('sizixian'))
				parent.window.frames[0].document.getElementById('sizixian').checked=true;	
			}else{
				for(var i=1;i<=1;i++){
					$("dsd"+i).style.display="none";
					$("dss"+i).style.display="none";
	...
	__int:function (){
		...
		parent.window.frames[0].document.getElementById('showselectnumber').innerHTML='数据生成中...';
	},
	__destruct:function (){
		this.__result='';
					   ...
					   for( var d = d_s; d < data[3].length; d++ ){
							...
							if( this.__check_hefen()&&this.__check_zhifanwei()&&this.__check_hanfushi()&&this.__check_chong()){
								...
								if( this.__check_dingwei_reg(numberstr) && this.__check_dan_reg(numberstr)&&this.__check_xiongdi_reg(numberstr)&&
									...
									if(this.__classid==1 && $('452ding').checked){
										numberstr = 'XXX' + numberstr.replace(/[xX]/g,'');
									}
									this.__result += this.__number_html(numberstr);;
									this.__selectnumber[this.__selectnumbertotal]=numberstr;
									this.__selectnumbertotal++;
...
nnnn++;
	...
	__innerHTML:function (shownumber){
		...
		objfra.document.getElementById('showselectnumber').innerHTML=html;
		//objfra.$("showselectnumber").innerHTML=shownumber;
		objfra.document.getElementById('selectnumber').value=this.__selectnumber;
		objfra.document.getElementById('selectnumbertotal').innerHTML=this.__selectnumbertotal;
		objfra.document.getElementById('selectnumbertotal_hidden').value=this.__selectnumbertotal;
		objfra.document.getElementById('selectlogs').value=this.__setselectlogs();
		//objfra.$("money").select();
		//alert(window.parent.frames["soonselectorder"].$("money").focus())
		setTimeout("parent.window.frames[0].document.getElementById('money').focus();",600);
	},
