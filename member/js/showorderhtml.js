...
function showhtml(json){
	...
	_tuimamode = json['m'];
	IN_WAPCG = in_wapcg;
	w1 = '10%';w2 = '15%';w3 = '12%';w4 = '12%';w5 = '12%';w6 = '12%';w7 = '12%';w8='15%';
	if(IN_WAPCG==1){
		ordernum=2;
		w1 = '';w2 = '';w3 = '35%';w4 = '30%';w5 = '35%';w6 = '';w7 = '';
	}
	var h='';
	h = '<form method="post" name=datamembers action="tuima.php?doactionfrom=main_ifr1">'
...
function set_order_print(joinstr,sTemp2,obj){
	...
	*/
	...
	//html += '<input type=button value="设置图示" name="printprex" id="printpre" class="Noprint" onClick="parent.main.location=\'membersetprint.php\';">';
	html += '';
	...
	// html += '  <tr class="print2" id="print2"><td style="text-align:center;" colspan=3><table id="add_neworder"></table></td>';
	i=0;allmoney=0;
	var getgj=str['j'];
	...
	for ( var j in gj) {
		...
		statsizi = val['statsizi']==1  || val['classid'].substr(0,1)==6 || val['classid'].substr(0,1)==7 ? "<span class=\"soon_b_f3\" style=\"FONT-SIZE:15px;color:red;\">现</span>":"";
		...
		html += '       <td style="text-align:center;">'+val['number']+statsizi+'</td><td style="text-align:center;">1:'+ Math.round(val['frank']*100)/100 +'</td><td style="text-align:center;">'+ Math.round(val['money']*100)/100 +'</td>';
		html += '      </tr>  ';
		i++;
		strid+=comm+val['id'];
		comm=',';
	...
	onclick1='ajax("POST","./ajax/ajax.php?action=soonprintstat",true,"strid=' + strid + '");';
	onclick2=' <input  class="Noprint"  type=button value="清 空" style="height:20;width=60" onclick=\''+onclick1+'location.href="left.php";\' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	onclick3=' XMLHttp.sendReq("GET","../ajax/ajax.php?action=memberprintstat'+_oidstr+'&strid='+strid+'","sid='+_sid+'&inajax=1",post_selectMsg,""); ';
	...
	html += ' <input  class="Noprint" name="numprint" id="numprint"  type=button value="打 印" style="height:20;width=60" onclick="setprint()";></td>';//'+onclick1+'
	html += '     </tr>';
...
function showprint(cur_page,arr){
	joinstr = eval('('+arr+')');
	iniPage(cur_page,joinstr,'');
  ...
  request.onreadystatechange = function(){
    if (request.readyState == 4 && request.status == 200){
      if (request.responseText){
          callbackFunction(iCurrPage,eval('('+JSON.parse(request.responseText)+')'),'');
      }
    }
  };
  request.send(vars);
...
function PrintPage(p){
	//PrintPageAjax("contral/ajax.php?action=printshow&iCurrPage="+p,"inajax=1",p,iniPage);
	PrintPageAjax("contral/ajax.php?action=printshow&cur_page="+p,"",p,iniPage);
}
...
function iniPage(iCurrPage,joinstr,obj){
	...
	if(obj==''){
		...
		if(gethtml!=false){
			document.getElementById('showorderhtml').innerHTML=gethtml;
		}
	}else{
		var gethtml = set_order_print(joinstr,sTemp2,obj);
		if(gethtml!=false) obj.$("showorderhtml").innerHTML=gethtml;
	} 
	//需要網下移時
