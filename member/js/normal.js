...
function go_select_two(v){
    $(".yellow").next("td").next("td").children("span").children(".inp1").val(v);
}
...
function get_synchro_rate(oid,t3,i,user_id,abcd_rate){
        $.ajax({
            type: "post",
            url: 'ajax/ajax_get_synchro_rate.php',
            data: {'o_id':oid,'t3':t3,'user_id':user_id,'abcd_rate':abcd_rate},
            success: function(data){
                $('.synchro_rate'+i).html(data);
            },
            error: function(){
                //alert('出错了!!');
            }
