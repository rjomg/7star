...
function showhtml(json){
	...
	var h='';
	h = '<form method="post" name=datamembers action="./index.php?action=ordertuima&doactionsend=soonorder" onsubmit="return false;">'
	+'<input type="hidden" name="formhash" value="'+_FORMHASH+'">'
	+'<input type="hidden" name="ordertuima_del" >'
	+ '<input type="hidden" id="PostOnlyRand" name="PostOnlyRand" value="'+OnlyNumber()+'" />';
	...
	if(IN_WAPCG==1 && gj.length>2){
		var newArray=gj.slice(-2); 
		gj=newArray;
		OldOrder=newArray;
	}else if(gj.length>_ordernum){
	...
	for ( var j in gj) {  
		...
		if(val['stattuima']==1){
			...
			tuima =(( _timestamp - _tuimatime) > val['datetime'] || val['yicangprint']==1||val['yicangread']==1)   ? '--':"<input type=\"checkbox\" name=\"idarray[]\" value=\""+val['id']+"\" class=\"checkbox\" >";
				...
				tuimastat=true;
			tuima =((( _timestamp - _tuimatime) > val['datetime'] || val['yicangprint']==1||val['yicangread']==1)   ? '--':"<a href=\"javascript:void(0)\"  onclick=\"javascript:if(window.confirm('确定退掉该号码吗?')){ if(tuimastat==true){tuimastat=false;window.parent.parent._OldOrderPrint=[];location.href='index.php?action=ordertuima&doactionsend=soonorder&idarray="+val['id']+"&PostOnlyRand="+OnlyNumber()+"'; }}else{ return;} return false;\"><font style='color:#009900'>退码</font></a>"); 
			}
		}
		...
		if (i==0){
			...
			if(_tuimamode==1){
				...
				h+= '<input class="button" style="padding: 0 1px;line-height: 22px;;height: 22px;height: 22px !important;" type="button" name="ordertuima_del_button" onclick="if(checkempty(this.form, \'idarray\')){if(tuimastat==true){tuimastat=false;window.parent.parent._OldOrderPrint=[];datamembers.ordertuima_del_button.disabled=true;datamembers.ordertuima_del.value=\'ordertuima_del\';datamembers.submit();}}" value="退码">';
				//
			}else{ h+='操作';}}
			h+= '</td><tr>';	
		}
		h+= '<tr '+class1+' style="height:28px;line-height:19px;">';
			if(IN_WAPCG!=1){h+= '<td >'+strcaizhong+'</td><td >'+val['orderid']+'</td>';}
...
function set_order_print(joinstr,sTemp2,obj){
   ...
   //var joinstr = eval('('+arr+')');
	...
	if(pages>0){
		...
		//var sss=pagenum+"="+$("test11").innerHTML;
		//$("test11").innerHTML=sss;
...
function IssuenoLogPage(p,n){
	PrintPageAjaxLog("./wap_ajax.php?action=issuenologpage&pagename="+n+"&page1="+p,"inajax=1",p,n,IssuenoLogIniPage);
}
function PrintPageAjaxLog(url, vars, iCurrPage,pagename ,callbackFunction){
	  var request = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("MSXML2.XMLHTTP.3.0");
	  request.open("GET", url, true); 
	  request.onreadystatechange = function(){
	    if (request.readyState == 4 && request.status == 200){
	      if (request.responseText){         
	          callbackFunction(iCurrPage,eval('('+request.responseText+')'),'',pagename);
	      }
	    }
	  };
	  request.send(vars);
	}
function IssuenoLogIniPage(iCurrPage,joinstr,obj,pagename){
	//alert(joinstr)
	//var join = eval('('+joinstr+')');
	
	var html = IssuenoLoghtml(joinstr);
	if(obj=='')$("showissuenohtml"+pagename).innerHTML=html;
}
...
function IssuenoLoghtml(json){
	...
	if(in_wapcg==1){
		...
		for ( var j in gj) { 	
			...
			statsizi = val['statsizi']==1 || val['classid']==6 || val['classid']==7? "<span class=\"soon_b_f3\">现</span>":"";
			html +='<input type="checkbox" name="idarray" value="'+j+'-'+val['id']+'" >'+val['number']+'='+val['money']+statsizi+'&nbsp;'+sbr;
			sbr='';
			i++;
		}
		html += '合计:'+tatolmoney+' ';
	...
	if(_mysendmode==1 && _tradstat==1){
		...
		for ( var j in gj) { 	
			...
			var colorlast2="";
			if (val['colorlast']==1){
				colorlast1="soon_b_f1";
				colorlast2="soon_b_f2";

			}
			html += '<td class="soon_b_B '+colorlast1+' '+class1+' '+class2+'" style="width:14.5%;height:28px;line-height:19px;">';
		...
		for ( var j in gj) { 	
			...
			html += '<tr '+class1+' '+class2+' title="'+val['datetime']+'" style="height:28px;line-height:19px;">'
				+'<td width="22%" class="soon_b_B '+colorlast2+'">'+val['number']+statsizi+'</td>'
				+'<td width="25%" class="soon_b_B '+colorlast1+'">'+val['money']+'</td>'
				+'<td width="23%">';
