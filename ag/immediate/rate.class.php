...
class rate0 extends mysql
{
		...
		function update_another_odd( $user_id, $oid, $plate_num, $o_content = "", $ab_content = "" )
		{
				...
				if ( $fxxk_id != 0 )
				{
						$dn = $this->select( "odds_set", "*", "o_id={$fxxk_id} and user_id={$user_id} and plate_num='{$plate_num}'" );
						$dm = $this->fetch_array( $dn );
						$do = $dm['o_content'];
						$doab = $dm['ab_content'];
						if ( strstr( $y['o_typename'], "B" ) )
						{
								$this->update( "odds_set", "ab_content='{$doab}'", "o_id={$oid} and user_id={$user_id} and plate_num='{$plate_num}'" );
						}
						...
						$n = $this->select( "odds_set", "*", "o_id={$oid} and user_id={$user_id} and plate_num='{$plate_num}'" );
						...
						$this->update( "odds_set", "o_content='{$fxxk_content}',ab_content='{$r}'", "o_id={$fxxk_id} and user_id={$user_id} and plate_num='{$plate_num}'" );
				}
		}
