...
if ($_GET['plate_num']) {
	$where_user.='orders.plate_num ='.$_GET["plate_num"];
}else{
	$where_user.='orders.plate_num ='.$plate_num;
}
...
//var_dump('select orders.*,users.user_name,users.user_nick from orders left join users on users.user_id=orders.user_id where '.$where_user.' order by time DESC limit '.$firstcount.','.$displaypg);exit;
...
<?php  if($_GET['is_win']==1){echo '<input type="hidden" name="is_win" value="1">';}  ?>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
						...
						<tr class="hover" onmouseover="hover1(this);" onmouseout="hover2(this);" style="height:28px;line-height:29px;">
							...
							<td class="<?php if($value['stattuima']==1){echo 'soon_tuima';}else{echo '';}?>" width="7%">
								...
								<?php 
									if ($my_power=='公司') {
										echo $xx_h=$value['orders_y']*$value['f_tui']+$value['orders_y']*$value['gd_tui']+$value['orders_y']*$value['zd_tui']+$value['orders_y']*$value['d_tui']+$value['orders_y']*$value['h_tui'];
									}
									if ($my_power=='分公司') {
