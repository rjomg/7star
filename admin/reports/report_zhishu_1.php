...
foreach ( $downuser_arr as $du => $downuser )
{
		...
		if ( $user_bishu )
		{
				...
				foreach ( $user_arrs as $us )
				{
						...
						$gaiusers = $db->select( "orders", "*", "user_id={$us} limit 0,1" );
						...
						if ( $u_power == 1 )
						{
								...
								$gongxian_top = 0;
								$yingshou_down = $shijijieguo1 - $xiajizuanqutuishui;
								$yingfu_top = 0;
						}
						else if ( $u_power == 2 )
						{
						...
						else if ( $u_power == 3 )
						{
								...
								$yingfu_top = $shijijieguo1 + $shijijieguo2 + $ttts;
						}
						else if ( $u_power == 4 )
						{
								...
								$yingfu_top = $shijijieguo1 + $shijijieguo2 + $shijijieguo3 + $ttts;
						}
						else if ( $u_power == 5 )
						{
								...
								$yingfu_top = $shijijieguo1 + $shijijieguo2 + $shijijieguo3 + $shijijieguo4 + $ttts;
						}
						$gongxian_top = round( $gongxian_top, 2 );
						$yingshou_down = round( $yingshou_down, 2 );
						$yingfu_top = round( $yingfu_top, 2 );
						$shitouzonge = $user_zonge - $user_tuishui;
						$user_sy = round( !$jiesuan ? "0" : $user_shuying + $user_tuishui, 2 );
						$shizhanzhue = round( $user_zonge * $gaiuser_percent / 100, 2 );
						$shizhanjieguo = round( !$jiesuan ? "0" : 0 - $user_sy * $gaiuser_percent / 100, 2 );
