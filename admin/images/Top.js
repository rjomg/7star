...
function Loading_But(bid1,bID) {
    ...
    for (i=0;i<(mBut.length);i++){
        if (mBut[i] instanceof Array) {
		    ...
		    But_Htm+="<td width='" + But_W + "' height='28'><table width='" + (But_W-5) + "' border='0' align='center' cellpadding='0' cellspacing='0'><tr><td height='20' style='background:none;border:none;cursor:hand' onMouseOver=this.style.backgroundImage='url(images/bg.gif)';this.style.borderStyle='none';this.style.borderWidth='1';borderColor='#a6d0e7'; onmouseout=this.style.backgroundImage='url()';this.style.borderStyle='none' onClick=" + mBut[i][1] + "><div align='center'" + Color_B + ">" + mBut[i][0] + "</div></td></tr></table></td>";
		}
