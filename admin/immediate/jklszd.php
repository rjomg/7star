...
$qishu['plate_num'] = $_GET['plate_num'];
if ( empty( $qishu['plate_num'] ) )
{
		$qishus = $db->select( "plate", "*", "1 order by plate_num desc limit 1" );
		$qishu = $db->fetch_array( $qishus );
}
$x = $db->select( "orders", "*", "plate_num={$qishu['plate_num']} order by id desc limit 0,100" );
