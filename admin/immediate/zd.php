...
$mypowername = $db3->get_user_power_char( $_SESSION["user_power".$c_p_seesion] + 1 );
...
foreach ( $y as $key => $row )
{
		...
		if ( empty( $user_name ) )
		{
				echo $_SESSION["username".$c_p_seesion]."<font color=blue>[走飞]</font>";
		}
		else
		{
				echo $user_name;
		...
		if ( !empty( $user_name ) )
		{
				echo "zd_down.php?power=";
				echo $_SESSION["user_power".$c_p_seesion] + 1;
		}
		else
		{
				echo "zd_huiyuan.php?power=6&is_fly=1";
		}
		echo "&user_id=";
		echo $key;
		echo "&qishu=";
