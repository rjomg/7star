...
// var_dump($plate['plate_time_satrt']);exit;
	...
	if ($_POST && $_GET['action']=='soonselect') {
		...
		foreach ($send_data as $key => $value) {
			...
			$oddsset=$db->get_one('select * from tuishui_set where user_id='.$uid.' and typename='.$o_type2);
			if (empty($oddsset)) {
				$oddsset=$db->get_one('select * from oddsset_type where user_id=0 and o_typename='.$o_type2);
				$orders['orders_p']=$oddsset['o_odd_limit'];//下注时赔率
				$orders['h_tui']=0; //会员退水
			}else{				
				$orders['orders_p']=$oddsset['oddsset'];//下注时赔率
				$orders['h_tui']=$value['money']*$oddsset['tuishui']; //会员退水
			}
	...
	if ($_GET['action']=='chacknumbermoney') {
		...
		if ($_GET['sizixian']=='1') {
			$oddsset=$db->get_one('select * from tuishui_set where user_id='.$uid.' and typename="四字定"');
			if (empty($oddsset)) {
				$oddsset=$db->get_one('select * from tuishui_set where user_id=0 and typename="四字定"');
			}
	...
	if ($_GET['action']=='numberfrank') {
		...
		$oddsset=$db->get_one('select * from tuishui_set where user_id='.$uid.' and typename='.$typename);
		if (empty($oddsset)) {
			$oddsset=$db->get_one('select * from tuishui_set where user_id=0 and typename='.$typename);
		}
	...
	if ($_GET['action']=='soonsend') {
		...
		$orders['o_type1']='四字定';
			...
			if (empty($oddsset)) {
				...
				$orders['orders_p']=$oddsset['oddsset'];//下注时赔率
				$orders['h_tui']=$post_n_m[1]*$oddsset['tuishui']; //会员退水
			}
			$orders['plate_num']=$plate['plate_num'];  //期数
